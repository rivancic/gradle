...
/**
 * Factory class that based on provided sorting type parameter returns concrete {@link FileDirectoryMapper}.
 */
  ...
  public static FileDirectoryMapper getFileDirectoryMapper(Logger logger, Provider<String> sortType) {
    if (sortType.isPresent()) {
      if (sortType.get().equals("extension")) {
        ...
        throw new InvalidUserDataException("Invalid property sortType value provided [" + sortType.get() + "]. Valid values are ['extension','date','alphabet']");
      }
    } else {
      if (logger != null) {
